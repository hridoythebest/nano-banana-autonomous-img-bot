...
def generate_ad(person_img: Path, product_img: Path, prompt: str, outdir: Path, model_name: str, api_key: str) -> Path | None:
    print(f"\n[Start] Compositing person='{person_img.name}' with product='{product_img.name}' using key ending with {api_key[-10:]}")
    try:
...
def main():
    ...
    if args.person and args.product:
        ...
        pairs.append((person_path, product_path, next(key_cycle)))
    else:
        ...
        for p in person_images:
            for q in product_images:
                pairs.append((p, q, next(key_cycle)))

    print(f"Total compositions to generate: {len(pairs)}")
    success = 0

    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = [executor.submit(generate_ad, pi, qi, args.prompt, outdir, args.model, api_key) for pi, qi, api_key in pairs]
